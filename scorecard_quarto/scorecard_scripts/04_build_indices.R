...
# PATHS
# -----------------------------
SCORED_PATH <- "/Users/heener/Documents/GitHub/ADAPARCDataPipeline/scorecard_quarto/scorecard_data/processing/indicator_scored_long_with_composites.csv"
INDEX_DICT_PATH <- "/Users/heener/Documents/GitHub/ADAPARCDataPipeline/scorecard_quarto/dictionary/scorecard_index_dictionary.csv"
...
# -----------------------------
# 2024 Revision note: the index dictionary intentionally references
# indicator_ids that are not yet in scored output (acs_needs_pull and
# external_placeholder indicators skipped by step 02). These will produce
# NA values in their parent indices via the min_frac_present logic, which
# is the correct behaviour for a partial build.
# Hard stop is replaced with a warning so the pipeline can run with
# ACS-ready indicators while placeholder data is pending.
...
if (length(missing_scorecard_members) > 0) {
  warning(
    glue(
      "{length(missing_scorecard_members)} scorecard_index member(s) not in scored output ",
      "(likely acs_needs_pull or external_placeholder — expected):\n- ",
